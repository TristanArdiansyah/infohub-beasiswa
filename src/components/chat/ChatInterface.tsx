...
import { ALL_CHAT_NODES } from '@/data/adem';
...
import { SearchResult } from '@/data/adem/searchIndex';
...
export default function ChatInterface() {
  ...
  const processNode = (nodeId: string) => {
    const node = ALL_CHAT_NODES[nodeId as keyof typeof ALL_CHAT_NODES];
  ...
  const currentNode = ALL_CHAT_NODES[currentNodeId as keyof typeof ALL_CHAT_NODES];
    ...
    <div className={`relative flex flex-col h-screen w-full max-w-xl mx-auto rounded-xl shadow-2xl overflow-hidden border border-white-200 transition-colors duration-500 backdrop-blur-md bg-slate-50`}>
      ...
      <div className={`${activeTheme.accent} transition-colors duration-500 p-4 text-white flex items-left justify-between shadow-md z-10 shrink-0`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold tracking-wide text-sm md:text-base">Asisten ADEM</h2>
        ...
        {history.length > 1 && (
          <button 
            onClick={() => { setHistory([]); processNode('start'); }}
